...
//    //TODO存如到mysql中 用foreach则太慢 一次性取一条
//    reduced.foreach(   r=>{
//    val con=DriverManager.getConnection("jdbc:mysql://node1:3306/bigdata","root","a")
//      val pstm=con.prepareStatement("insert into ipresult(province,visits) values(?,?)")
//      pstm.setString(1,r._1)
//      pstm.setInt(2,r._2)
//    }     )
    reduced.foreachPartition(it => {
      val con=DriverManager.getConnection("jdbc:mysql://node1:3306/bigdata","root","a")
      val pstmt=con.prepareStatement("insert into ipresult(province,visits) values(?,?)")
      it.foreach( r=>{
        pstmt.setString(1,r._1)
        pstmt.setInt(2,r._2)
        pstmt.executeUpdate()
      })
    })//这段处理是在driver端运行的
